...
On the visual side of things, each window gets its own <<_the_statusline,"`statusline`">> and, only in GVim/MacVim, its own scrollbar.

== Tab pages

In the same way windows are an abstraction built on top of buffers, tab pages are an abstraction built on top of windows.

Tab pages--too often called "`tabs`"--have been a neverending source of confusion since their appearance in Vim 7.x. Despite their name and how the <<_the_tabline,"`tabline`">> widget is implemented, Vim's _tab pages_ are not like other editor's _tabs_.
...
Buffers, window, and tab pages are explored thoroughly link:02.html[in the following chapter].
...
The quickfix list is a handy list of addresses that we can populate and navigate at will. Each entry in the list is roughly made of:
...
That list is global so you can't have more than one active at the same time but it's pefectly possible to recall previous lists with `:colder`.
...
Line numbering is a fairly straightforward feature that doesn't deserve much explanation. One enables it with `:set number`, disables it with `:set nonumber` and decides how much room it takes with `:set numberwidth=6`.
