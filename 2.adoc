...
It is also possible to define colors for specific parts but I'd recommend measure, here: the statusline is an information device, not a decorative widget.
...
Line numbers are disabled by default but we already enabled them in a previous chapter. They are a fairly straightforward feature that doesn't warrant much explanation. One enables it with `:set number`, disables it with `:set nonumber` and decides how much room they take with `:set numberwidth=6`.
