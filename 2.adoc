...
But we are not limited to absolute line numbers, though: we can use relative line numbers to see how far a line is from the current line.
...
The "sign column" can't be shown or hidden manually: Vim shows it when you have one or more signs in your buffer and removes it when there's no sign anymore.
