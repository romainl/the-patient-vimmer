...
<<<<<<< HEAD
At the root of the tree -- and at the center of Vim's design -- is the buffer; a little higher on the abstraction ladder is the window and, higher again, is the tab page. Vim will quit when you delete the last buffer, close the last window or close the last tab page.

== Buffers

Like Word or Photoshop, Vim lets us create, delete and -- of course -- edit "`buffers`" (Vim's lingo for "`documents`"). Like in Word or Photoshop, those buffers can be associated with an actual file or not and your changes will not be written to disk unless you instruct Vim to do so with `:write` or `:update`. Like Word or Photoshop, Vim will tell you when you are trying to quit without saving... No, Vim is not _that_ weird, after all.
=======
...
<<<<<<< HEAD
Every buffer created in Vim via `:edit`, `:find`, `:args`, any similar command or simply `$ vim filename` is added to the _buffer list_ where it is assigned a number that will never change until it is explicitely wiped out. Beside that number, each buffer may have a name:

* when it is associated with a file, the buffer is named after that file,
* when it was created with a command like `:e foobar`, and there's no file named `foobar` in the working directory, the buffer is named `foobar`
=======
...
One way to avoid thinking too much about your buffers would be to mash `:bn[ext]` and `:bp[revious]` until you get where you want. Your destination might be sixteen buffers away, though, so cycling is not a silver bullet.

But that boring `:ls`, then `<CR>`, then `:b`, then `24`, then `<CR>` routine looks like a good candidate for a custom mapping. http://of-vim-and-vigor.blogspot.fr/p/vim-vigor-comic.html[Flying is faster than cycling], right?

<<<<<<< HEAD
image::flying.png[link="http://of-vim-and-vigor.blogspot.fr/p/vim-vigor-comic.html"]

Here is a nice jet-pack for our arsenal:
=======
...
As demonstrated above, buffers can be addressed by their number but also by their name. Just like with `:edit` and `:find`, we can tab-complete buffer names on the command-line and -- why not? -- add yet another mapping to our `vimrc`:
...
We shouldn't be concerned by buffers only for navigation, though: Vim allows us to perform actions _on each buffer in the buffer list_ with the `:bufdo` command which can be a powerful tool:
...
<<<<<<< HEAD
As briefly mentioned in the introduction, we _always_ have at least one window in Vim, that _always_ displays one buffer.

The purpose of a window is to provide a view into a buffer. It can be created, moved around and closed but it is important to remember that windows and buffers are not coupled **at all**.

Since any buffer could end up being displayed in any given window it is plain wrong to assume "`switching to a window`" to be the same as "`switching to a buffer`", let alone "`switching to a file`".
