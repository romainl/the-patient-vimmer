...
which allows us to find files in the directory of the current file *and* anywhere under the working directory, recursively, without needing to use `**` explicitly.
...
NOTE: like `:edit`, `:find` does its completion from the start of the file name so `:find foo` will match `foobar.txt` but not `model_foo.txt`. Add a wildcard for an even more useful completion:
...
The file names used as arguments when starting Vim are stored in the global *argument list* which can be manipulated. In practice editing multiple files at once is as simple as replacing the argument list:
